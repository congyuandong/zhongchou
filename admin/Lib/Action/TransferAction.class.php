...
class TransferAction extends CommonAction{
	...
	public function add()
	{
		...
		$this->assign("new_sort", M("Transfer")->max("sort")+1);
		$this->display();
	}
	...
	public function insert() {
		...
		if (false !== $list) {
			...
			{
				$deal_count = M("Transfer")->where("user_id=".$data['user_id']." and is_effect = 1 and is_delete = 0")->count();
	...
	public function set_sort()
	{
		...
		$log_info = M("Transfer")->where("id=".$id)->getField("name");
		if(!check_sort($sort))
		{
			$this->error(l("SORT_FAILED"),1);
		}
		M("Transfer")->where("id=".$id)->setField("sort",$sort);
