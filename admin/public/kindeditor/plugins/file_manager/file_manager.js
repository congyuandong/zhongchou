...
function delImg(filename)
{
	...
	{
		location.href = (root_path+"?m=File&a=deleteImg&file="+filename);
	}
}
if (!this.JSON) {
	this.JSON = {};
...
KE.event.ready(function() {
	...
	var createList = function(responseText) {
		...
		for (var i = 0, len = fileList.length; i < len; i++) {
			...
			cell3.className = 'op';
			cell3.innerHTML = "<span style='cursor:pointer;' onclick=\"delImg('"+fileUrl+"');\">删除</span>";
			}
		}
	};
	var createView = function(responseText) {
		...
		for (var i = 0, len = fileList.length; i < len; i++) {
			...
			if(!data.is_dir)
			{
				var opDiv = KE.$$('div', document);
				opDiv.className = 'div';
				opDiv.title = "删除";
				opDiv.innerHTML = "<div style='cursor:pointer; text-align:center;' onclick=\"delImg('"+fileUrl+"');\">删除</div>";
